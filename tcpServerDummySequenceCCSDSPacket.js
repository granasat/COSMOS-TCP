...
var server = net.createServer(function(socket) {            //Callback function called when 'connection' event of net is called
    ...
    var interval = setInterval(function () {
        //First Packet
        var date = new Date();
        var data_field = Buffer.from([date.getSeconds(), date.getMinutes(), date.getHours()]);
        var header = createHeader(primHeader.version, primHeader.pType, primHeader.secHeaderFlag, 0, 0x01,
        ...
        //Last Packet
        var date = new Date();
        var data_field = Buffer.from([date.getDay(), date.getMonth(), date.getFullYear()]);
        var header = createHeader(primHeader.version, primHeader.pType, primHeader.secHeaderFlag, 0, 0x10,
            primHeader.seqCount, (data_field.length - 1).toString(2));      //Length minus 1 see doc
        socket.write(Buffer.concat([header, data_field], header.length + data_field.length));     //Write the header first and then the data
    }, 1000);
